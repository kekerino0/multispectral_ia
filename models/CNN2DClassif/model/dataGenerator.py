...
class DataGenerator(Sequence):
    def __init__(self, label_dir, image_dir, max_num_box, num_classes, list_of_image_paths, list_of_label_paths, batch_size=32, image_size=(256, 256)):
        ...
        self.indexes = np.arange(len(self.list_of_image_paths))
    ...
    def __getitem__(self, index):
        ...
        batch_image_paths = [self.list_of_image_paths[i] for i in batch_indices]
        batch_label_paths = [self.list_of_label_paths[i] for i in batch_indices]
    ...
    def load_and_preprocess_label(self, label_path):
        with open(self.label_dir + '\\' +label_path ,'r') as label_file:
            print('path : ',self.label_dir + '\\' +label_path)
...
def augment_data(dataset, shuffle=False, augment=False):
    if augment : 
        augmentedDS = dataset.map(lambda x, y:(data_augmentation(x, training=True), y), 
                num_parallel_calls=AUTOTUNE)
    return augmentedDS.prefetch(buffer_size=AUTOTUNE)
'''
